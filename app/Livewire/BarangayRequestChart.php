...
class BarangayRequestChart extends ChartWidget
{
    protected static ?string $heading = 'Barangay ID';
    ...
    protected function getData(): array
    {
        $monthlyDocuments = DB::table(DB::raw('(SELECT 1 AS month UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12) AS months'))
            ->leftJoin(DB::raw('(SELECT MONTH(created_at) as month, COUNT(*) as total FROM request_documents WHERE YEAR(created_at) = YEAR(CURRENT_DATE) AND type = "barangay_id"  GROUP BY month) AS documents'), 'months.month', '=', 'documents.month')
